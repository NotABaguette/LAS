...
Debian Router is split into three layers:
...
- Match fields: input/output link, source/destination CIDRs, source/destination ports, protocols, tunnel IDs, domains, and GeoIP tags.
- Action fields: direct, interface, tunnel, blackhole, reject, scan, mirror.
- Optional mark/table overrides.

For tunnel actions, the plan uses nftables to set a Linux fwmark, then installs `ip rule` entries to route marked traffic through the selected table.
...
- Service-backed tunnels: OpenVPN, IPsec/L2TP, VLESS, VMess, XHTTP, and custom tunnels are managed by their native daemons, and Debian Router routes traffic into the exposed interface or local TUN.
